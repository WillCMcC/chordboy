...
import ClockWorker from "../workers/clockWorker.ts?worker";
import type { StepAction } from "../types";

/** MIDI clock pulses per quarter note (standard 24 PPQN) */
const MIDI_CLOCKS_PER_BEAT = 24;

/** Clock callbacks for external MIDI sync */
...
/** Options for useTransport hook */
export interface UseTransportOptions {
  /** Callback when a preset should be triggered */
  onTriggerPreset?: (preset: string) => void;
  /** Callback when a preset should be retriggered */
  onRetriggerPreset?: (preset: string) => void;
  /** Callback to stop all playing notes */
  onStopNotes?: () => void;
  /** Function to set clock callbacks in MIDI provider */
  setClockCallbacks?: (callbacks: ClockCallbacks) => void;
}

/** Internal state container for callbacks */
...
interface WorkerMessage {
  type: "start" | "stop" | "setBpm";
  payload?: { bpm: number };
}

/** Return type for useTransport hook */
export interface UseTransportReturn {
  ...
  // Sequencer actions
  setSequencerEnabled: (value: boolean | ((prev: boolean) => boolean)) => void;
  setSequencerSteps: (value: number | ((prev: number) => number)) => void;
  setStepsPerBeat: (value: number | ((prev: number) => number)) => void;
  setRetrigMode: (value: boolean | ((prev: boolean) => boolean)) => void;
  setStep: (stepIndex: number, presetSlot: string | null) => void;
...
export function useTransport(
  { onTriggerPreset, onRetriggerPreset, onStopNotes, setClockCallbacks }: UseTransportOptions = {}
): UseTransportReturn {
  const [bpm, setBpm] = useState<number>(120);
  const [isPlaying, setIsPlaying] = useState<boolean>(false);
  const [currentBeat, setCurrentBeat] = useState<number>(0); // 0-3 for visual display
  const [syncEnabled, setSyncEnabled] = useState<boolean>(false); // Whether to sync to external MIDI clock

  // Sequencer state
  const [sequencerEnabled, setSequencerEnabledState] = useState<boolean>(false);
  const [sequencerSteps, setSequencerStepsState] = useState<number>(8); // Number of steps (4, 8, 16)
  const [currentStep, setCurrentStep] = useState<number>(0);
  const [sequence, setSequenceState] = useState<(string | null)[]>([]); // Array of preset slots (or null for empty)
  const [stepsPerBeat, setStepsPerBeatState] = useState<number>(1); // 1 = quarter notes, 2 = eighth notes, 4 = sixteenth
  const [retrigMode, setRetrigModeState] = useState<boolean>(true); // true = retrigger same notes, false = sustain
  const [isLoaded, setIsLoaded] = useState<boolean>(false); // Track if we've loaded from storage
  ...
  const stepPulseCountRef = useRef<number>(0);
  const lastTriggeredPresetRef = useRef<string | null>(null); // Track last triggered preset for sustain mode

  // BPM calculation from external clock
  const lastClockTimeRef = useRef<number>(0);
  const clockTimesRef = useRef<number[]>([]); // Rolling window of clock intervals

  // State container - single ref that holds all values callbacks need
  // This replaces 6+ individual ref-sync useEffect hooks
  const stateRef = useRef<StateContainer>({
  ...
  const setSequencerEnabled = useCallback((value: boolean | ((prev: boolean) => boolean)): void => {
  ...
  const setSequencerSteps = useCallback((value: number | ((prev: number) => number)): void => {
    const newValue = typeof value === "function"
  ...
  const setStepsPerBeat = useCallback((value: number | ((prev: number) => number)): void => {
    const newValue = typeof value === "function"
  ...
  const setRetrigMode = useCallback((value: boolean | ((prev: boolean) => boolean)): void => {
    const newValue = typeof value === "function"
  ...
  const setSequence = useCallback((value: (string | null)[] | ((prev: (string | null)[]) => (string | null)[])): void => {
    const newValue = typeof value === "function"
   ...
   */
  const processPulse = useCallback((): void => {
    ...
    if (stateRef.current.sequencerEnabled) {
      ...
      if (stepPulseCountRef.current >= pulsesPerStep) {
        ...
        setCurrentStep((prev) => {
          ...
          // Execute the action
          switch (result.action as StepAction) {
            case "trigger":
   ...
   */
  const handleExternalClock = useCallback((): void => {
   ...
   */
  const handleExternalStart = useCallback((): void => {
    setIsPlaying(true);
    setCurrentBeat(0);
    setCurrentStep(0);
    pulseCountRef.current = 0;
   ...
   */
  const handleExternalStop = useCallback((): void => {
    setIsPlaying(false);
    setCurrentBeat(0);
    setCurrentStep(0);
    pulseCountRef.current = 0;
  ...
  const processPulseRef = useRef<() => void>(processPulse);
  ...
  useEffect(() => {
    if (clockWorkerRef.current) {
      clockWorkerRef.current.postMessage({ type: "setBpm", payload: { bpm } } as WorkerMessage);
    }
  }, [bpm]);
  ...
  const start = useCallback((): void => {
    ...
    if (clockWorkerRef.current) {
      clockWorkerRef.current.postMessage({ type: "start", payload: { bpm } } as WorkerMessage);
    }
  ...
  const stop = useCallback((): void => {
    ...
    if (clockWorkerRef.current) {
      clockWorkerRef.current.postMessage({ type: "stop" } as WorkerMessage);
    }
  ...
  const updateBpm = useCallback((newBpm: number): void => {
    const clamped = Math.max(20, Math.min(300, newBpm));
    setBpm(clamped);
  }, []);

  // Set a step in the sequence
  const setStep = useCallback((stepIndex: number, presetSlot: string | null): void => {
    setSequence((prev) => {
      const newSeq = [...prev];
  ...
  // Clear a step
  const clearStep = useCallback((stepIndex: number): void => {
    setSequence((prev) => {
      const newSeq = [...prev];
