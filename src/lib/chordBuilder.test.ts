...
import type { MIDINote, DropType } from "../types";
...
describe("chordBuilder", () => {
  ...
  describe("invertChord", () => {
    ...
    it("should handle null/undefined", () => {
      expect(invertChord(null as unknown as MIDINote[], 1)).toBe(null);
      expect(invertChord(undefined as unknown as MIDINote[], 1)).toBe(undefined);
    });
  ...
  describe("spreadVoicing", () => {
    ...
    it("should return original notes for unknown voicing type", () => {
      ...
      expect(spreadVoicing(notes, "unknown" as DropType)).toEqual([60, 64, 67, 71]);
    });
  });
});
