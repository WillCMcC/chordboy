...
describe("voicingTransforms", () => {
  describe("applyProgressiveDrop", () => {
    it("should return original notes for dropCount 0", () => {
      const notes: MIDINote[] = [60, 64, 67, 71];
      expect(applyProgressiveDrop(notes, 0)).toEqual([60, 64, 67, 71]);
    });

    it("should return original notes for null/undefined", () => {
      expect(applyProgressiveDrop(null as unknown as MIDINote[], 1)).toBe(null);
      expect(applyProgressiveDrop(undefined as unknown as MIDINote[], 1)).toBe(undefined);
    });
    ...
    it("should drop highest note down an octave for dropCount 1", () => {
      const notes: MIDINote[] = [60, 64, 67, 71]; // C, E, G, B
    ...
    it("should not drop all notes (leave at least one in place)", () => {
      const notes: MIDINote[] = [60, 64, 67]; // 3 notes
    ...
    it("should handle unsorted input", () => {
      const notes: MIDINote[] = [67, 60, 71, 64]; // Unsorted C, E, G, B
      const result = applyProgressiveDrop(notes, 1);
      expect(result).toEqual([59, 60, 64, 67]);
    });
    ...
    it("should create proper drop2 voicing", () => {
      ...
      const notes: MIDINote[] = [60, 64, 67, 71]; // C, E, G, B in close position
      const result = applyProgressiveDrop(notes, 1);
  ...
  describe("applySpread", () => {
    ...
    it("should return original notes for null/undefined", () => {
      expect(applySpread(null as unknown as MIDINote[], 1)).toBe(null);
      expect(applySpread(undefined as unknown as MIDINote[], 1)).toBe(undefined);
    });
    ...
    it("should handle unsorted input", () => {
      const notes: MIDINote[] = [67, 60, 71, 64]; // Unsorted
      const result = applySpread(notes, 1);
