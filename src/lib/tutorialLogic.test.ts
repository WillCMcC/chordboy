...
import type { TutorialState, MIDINote, NoteName } from "../types";

/** Test chord data structure */
interface TestChordData {
  root: NoteName;
  notes: MIDINote[];
  name: string;
}
...
interface DefaultTestParams {
  currentChord: TestChordData | null;
...
describe("tutorialLogic", () => {
  ...
  describe("MOBILE_TUTORIAL_STEPS", () => {
    ...
    it("should use same condition checks as desktop", () => {
      const state: Partial<TutorialState> = { hasRoot: true, isMinor: true, hasExtension: true };
      expect(MOBILE_TUTORIAL_STEPS[1].check?.(state as TutorialState)).toBe(true); // root
      expect(MOBILE_TUTORIAL_STEPS[2].check?.(state as TutorialState)).toBe(true); // quality
      expect(MOBILE_TUTORIAL_STEPS[3].check?.(state as TutorialState)).toBe(true); // extension
    });
  });

  describe("buildTutorialState", () => {
    const defaultParams: DefaultTestParams = {
      currentChord: null,
      inversionIndex: 0,
      octave: 4,
      spreadAmount: 0,
    ...
    it("should detect hasRoot when chord has root", () => {
      const state = buildTutorialState({
        ...defaultParams,
        currentChord: { root: "C" as NoteName, notes: [60], name: "C" },
      });
    ...
    it("should detect isMinor for minor chords", () => {
      ...
      for (const name of minorNames) {
        const state = buildTutorialState({
          ...defaultParams,
          currentChord: { root: "C" as NoteName, notes: [60, 63, 67], name },
        });
    ...
    it("should not detect isMinor for major chords", () => {
      const state = buildTutorialState({
        ...defaultParams,
        currentChord: { root: "C" as NoteName, notes: [60, 64, 67], name: "C Maj" },
      });
    ...
    it("should detect hasExtension for 7th chords", () => {
      ...
      for (const name of extensionNames) {
        const state = buildTutorialState({
          ...defaultParams,
          currentChord: { root: "C" as NoteName, notes: [60, 64, 67, 70], name },
        });
  ...
  describe("isStepConditionMet", () => {
    it("should return false for step 0 (no condition)", () => {
      const state: Partial<TutorialState> = { hasRoot: true };
      expect(isStepConditionMet(0, state as TutorialState)).toBe(false);
    });

    it("should return true for step 1 when hasRoot", () => {
      const state: Partial<TutorialState> = { hasRoot: true };
      expect(isStepConditionMet(1, state as TutorialState)).toBe(true);
    });

    it("should return false for step 1 when no root", () => {
      const state: Partial<TutorialState> = { hasRoot: false };
      expect(isStepConditionMet(1, state as TutorialState)).toBe(false);
    });

    it("should return true for step 2 when isMinor", () => {
      const state: Partial<TutorialState> = { isMinor: true };
      expect(isStepConditionMet(2, state as TutorialState)).toBe(true);
    });

    it("should return true for step 3 when hasExtension", () => {
      const state: Partial<TutorialState> = { hasExtension: true };
      expect(isStepConditionMet(3, state as TutorialState)).toBe(true);
    });

    it("should return true for step 4 when hasVoicingChange", () => {
      const state: Partial<TutorialState> = { hasVoicingChange: true };
      expect(isStepConditionMet(4, state as TutorialState)).toBe(true);
    });

    it("should return true for step 5 when hasPreset", () => {
      const state: Partial<TutorialState> = { hasPreset: true };
      expect(isStepConditionMet(5, state as TutorialState)).toBe(true);
    });

    it("should return false for step 6 (no condition)", () => {
      const state = {} as TutorialState;
  ...
  describe("isStepConditionMet with custom steps", () => {
    it("should work with mobile steps", () => {
      const state: Partial<TutorialState> = { hasRoot: true };
      expect(isStepConditionMet(1, state as TutorialState, MOBILE_TUTORIAL_STEPS)).toBe(true);
    });
  ...
  describe("full tutorial flow simulation", () => {
    it("should progress through all steps with correct actions", () => {
      ...
      expect(isStepConditionMet(0, {} as TutorialState)).toBe(false);
      ...
      state = buildTutorialState({
        currentChord: { root: "C" as NoteName, notes: [60, 63, 67], name: "C min" },
        inversionIndex: 0,
        octave: 4,
        spreadAmount: 0,
        presetCount: 0,
      ...
      state = buildTutorialState({
        currentChord: { root: "C" as NoteName, notes: [60, 64, 67, 70], name: "C 7" },
        inversionIndex: 0,
        octave: 4,
        spreadAmount: 0,
        presetCount: 0,
      ...
      expect(isStepConditionMet(6, {} as TutorialState)).toBe(false);
      expect(getNextStep(6)).toBe(null);
    });
  });
});
