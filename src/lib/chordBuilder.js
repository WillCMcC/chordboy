 ...
 * @param {Object} options - Additional options (octave, etc.)
 * @returns {Object} Chord object { root, intervals, notes, quality }
 */
export function buildChord(root, modifiers = [], options = {}) {
  ...
  const intervals = [INTERVALS.UNISON];
  ...
  const notes = buildNotesFromIntervals(root, octave, uniqueIntervals);
 ...
 * @param {number} inversionIndex - Which inversion (0 = root position, 1 = first inversion, etc.)
 * @returns {Array<number>} Inverted chord notes
 */
export function invertChord(notes, inversionIndex = 0) {
 ...
 */
export function getInversionCount(notes) {
 ...
 * @param {Array<number>} notes - Array of MIDI notes
 * @param {string} voicingType - Type of voicing ("drop2", "drop3", etc.)
 * @returns {Array<number>} Spread chord notes
 */
export function spreadVoicing(notes, voicingType = "drop2") {
