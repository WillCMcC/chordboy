...
import type { Chord, ModifierType, NoteName } from "../types";
...
describe("chordNamer", () => {
  ...
  describe("getSimpleChordName", () => {
    it("should return empty string for null chord", () => {
      ...
      expect(getSimpleChordName({} as Chord)).toBe("");
    });

    it("should return chord name from chord object", () => {
      const chord: Partial<Chord> = {
        root: "C" as NoteName,
        modifiers: ["minor", "dom7"] as ModifierType[],
      };
      expect(getSimpleChordName(chord as Chord)).toBe("C min7");
    });
  });
